...
- `WINEDLLOVERRIDES=dwmapi=n,b` is required for mods (Palisade sets it). UE4SS ships as a dwmapi.dll proxy; without the override Wine loads its builtin dwmapi and the framework sits inert. Safe when no proxy is on disk.
