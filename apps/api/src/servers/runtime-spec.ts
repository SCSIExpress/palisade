 ...
 */
/**
 * Patch Project Zomboid's <server>_SandboxVars.lua with the catalog settings the
 * user has ACTUALLY SET, upserting `Key = value,` lines in place — including keys
 * nested in the Map / ZombieLore / ZombieConfig sub-tables (emitAs "Table.Key").
 * Everything untouched keeps what the preset generated / admins tuned in-game.
 * The file is GENERATED by the game from the sandbox preset on first boot, so the
 * config-writer only patches it once it exists (sandbox tweaks apply from the
 * second start) — and "Re-apply preset on start" will overwrite them by design.
 */
export function patchZomboidSandboxVars(
  lua: string,
  ...
  const render = (raw: unknown): string => {
    ...
    // Strings are quoted; strip quotes/newlines that would corrupt the Lua.
    return `"${String(raw).replace(/[\r\n"]/g, " ").trim()}"`;
  };
  // Upsert `key = value,` inside doc[start..end). Returns the updated document.
  const upsert = (doc: string, start: number, end: number, key: string, value: string): string => {
    const slice = doc.slice(start, end);
    const re = new RegExp(`^(\\s*)${key} = .*?,?$`, "m");
    const m = slice.match(re);
    if (m && m.index !== undefined) {
      const line = `${m[1]}${key} = ${value},`;
      return doc.slice(0, start) + slice.replace(re, line) + doc.slice(end);
    }
    // Key absent (older build / trimmed file) → insert before the block's closer.
    const closer = slice.lastIndexOf("}");
    if (closer === -1) return doc;
    const insert = `    ${key} = ${value},\n`;
    return doc.slice(0, start + closer) + insert + doc.slice(start + closer);
  };
  // Locate a sub-table's block, e.g. "ZombieLore = {" ... its closing "}".
  const tableRange = (doc: string, table: string): [number, number] | null => {
    const open = doc.search(new RegExp(`^\\s*${table} = \\{`, "m"));
  ...
  for (const def of input.catalog.settings) {
    ...
    const value =
      def.type === "enum" || def.type === "int" || def.type === "float"
        ? String(Number(raw))
        : render(raw);
    const path = (def.emitAs ?? def.key).split(".");
    if (path.length === 2) {
      const range = tableRange(doc, path[0]!);
      if (range) doc = upsert(doc, range[0], range[1], path[1]!, value);
    } else {
      doc = upsert(doc, 0, doc.length, path[0]!, value);
    }
  }
  return doc;
}
